...
class FakePermit:
    def __init__(
        ...
        *,
        ...
        reservation: object = PermitReservation("fake-reservation"),
...
class FakeHTTPSConnection:
    ...
    def request(
        ...
        **kwargs: object,
        ...
        del kwargs
        self.requests.append((method, selector, body, dict(headers or {})))
        if self.request_error is not None:
            raise self.request_error

    def getresponse(self) -> FakeUrllibResponse:
...
class OpenRouterAdapterTests(unittest.TestCase):
    ...
    def test_malformed_permit_results_are_fail_closed_before_send(self) -> None:
        for reservation in (None, False, True, 0, "", [], {}, object()):
            with self.subTest(reservation_type=type(reservation).__name__):
                permit = FakePermit(reservation=reservation)
                transport = FakeTransport(FakeResponse(200, response_body()))
                result = invoke(permit=permit, transport=transport)
                self.assertFalse(result.ok)
                self.assertEqual(result.failure.category, "invalid_reservation")
                self.assertEqual(result.failure.accounting_state, "admission_denied")
                self.assertEqual(transport.calls, 0)
                self.assertEqual(permit.settle_calls, [])

    def test_typed_permit_result_is_the_only_admission_success_shape(self) -> None:
        permit = FakePermit(reservation=PermitReservation("fake-typed-reservation"))
        transport = FakeTransport(FakeResponse(200, response_body()))
        result = invoke(permit=permit, transport=transport)
        self.assertTrue(result.ok)
        self.assertEqual(transport.calls, 1)
        self.assertEqual(permit.settle_calls[0][0], permit.reservation)
    ...
    def test_accounting_failure_after_send_never_reports_success_or_refund(self) -> None:
        ...
        self.assertFalse(result.ok)
        self.assertEqual(result.failure.category, "accounting_failure")
        self.assertEqual(result.failure.original_category, "completed")
        self.assertEqual(result.failure.sent_state, "sent")
        self.assertEqual(result.failure.accounting_state, "settlement_failed")
        self.assertEqual(transport.calls, 1)

    def test_settlement_failure_preserves_original_provider_failure_category(self) -> None:
        permit = FakePermit(settle_error=RuntimeError("fake-accounting-secret"))
        transport = FakeTransport(FakeResponse(503, b'{"error":{}}'))
        result = invoke(permit=permit, transport=transport)
        self.assertFalse(result.ok)
        self.assertEqual(result.failure.category, "accounting_failure")
        self.assertEqual(result.failure.original_category, "provider_http_error")
        self.assertEqual(result.failure.accounting_state, "settlement_failed")
    ...
    def test_openrouter_optional_metadata_fixture_is_accepted_and_discarded(self) -> None:
        private_reasoning = "fake-private-reasoning-that-must-not-escape"
        private_annotation = "fake-private-annotation-text"
        body = response_body(
            message_extra={
                "refusal": None,
                "reasoning": private_reasoning,
                "reasoning_details": [{"type": "reasoning.text", "text": private_reasoning}],
                "annotations": [{"type": "url_citation", "text": private_annotation}],
            },
            outer_extra={
                "citations": ["https://fixture.invalid/citation"],
        ...
        self.assertNotIn(private_annotation, repr(result))

    def test_refusal_and_missing_content_remain_safe_failures(self) -> None:
        refusal = response_body(message_extra={"refusal": "fake refusal"})
        missing_content = json.loads(response_body())
        del missing_content["choices"][0]["message"]["content"]
        fixtures = (
            (refusal, "refusal_not_allowed"),
            (json.dumps(missing_content, separators=(",", ":")).encode(), "content_missing"),
        )
        for body, category in fixtures:
            with self.subTest(category=category):
                result = invoke(transport=FakeTransport(FakeResponse(200, body)))
    ...
    def test_optional_metadata_shapes_are_bounded(self) -> None:
        fixtures = (
            (response_body(message_extra={"reasoning": []}), "malformed_reasoning"),
            (
                response_body(
                    message_extra={
                        "reasoning": "x" * (openrouter.MAX_REASONING_BYTES + 1)
                    }
                ),
                "malformed_reasoning",
            ),
            (response_body(message_extra={"annotations": {}}), "malformed_annotations"),
            (response_body(outer_extra={"citations": {}}), "malformed_response"),
            (
                response_body(
                    outer_extra={
                        "citations": [None]
                        * (openrouter.MAX_OPTIONAL_METADATA_ITEMS + 1)
                    }
                ),
                "malformed_response",
            ),
            (response_body(outer_extra={"system_fingerprint": 1}), "malformed_response"),
        )
        for body, category in fixtures:
            with self.subTest(category=category):
                result = invoke(transport=FakeTransport(FakeResponse(200, body)))
    ...
    def test_real_urllib_path_sends_fixed_request_without_environment_proxy(self) -> None:
        response = FakeUrllibResponse(200, response_body())
        clock = FakeClock()
        FakeHTTPSConnection.instances = []
        FakeHTTPSConnection.response = response
        FakeHTTPSConnection.request_error = None
        permit = FakePermit(on_reserve=lambda: setattr(clock, "now", 112.0))
        with (
            patch.object(http.client, "HTTPSConnection", FakeHTTPSConnection),
            ...
            result = invoke(
                permit=permit,
                clock=clock,
                transport=openrouter.UrllibTransport(),
            )
        ...
        self.assertGreater(connection.timeout, 5.0)
        self.assertEqual(connection.timeout, 18.0)
        self.assertEqual(len(connection.requests), 1)
        method, selector, body, headers = connection.requests[0]
        self.assertEqual(method, "POST")
        self.assertEqual(selector, "/api/v1/chat/completions")
        ...
        self.assertEqual(lower_headers["authorization"], f"Bearer {FAKE_CREDENTIAL}")
        self.assertEqual(lower_headers["content-type"], "application/json")
        self.assertEqual(lower_headers["accept"], "application/json")
        self.assertEqual(int(lower_headers["content-length"]), len(body))
        self.assertEqual(response.read_limits, [MAX_RESPONSE_BYTES + 1])
        self.assertTrue(response.closed)

    def test_real_urllib_timeout_is_unknown_sent_without_retry(self) -> None:
        FakeHTTPSConnection.instances = []
        FakeHTTPSConnection.response = None
        FakeHTTPSConnection.request_error = TimeoutError()
        permit = FakePermit()
        with (
            patch.object(http.client, "HTTPSConnection", FakeHTTPSConnection),
            patch.object(socket, "socket", side_effect=AssertionError("socket used")),
        ):
            result = invoke(permit=permit, transport=openrouter.UrllibTransport())

        self.assertFalse(result.ok)
        self.assertEqual(result.failure.category, "timeout")
        self.assertEqual(result.failure.sent_state, SentState.UNKNOWN.value)
        self.assertEqual(permit.settle_calls[0][1], AttemptOutcome.UNKNOWN_SENT)
        self.assertEqual(len(FakeHTTPSConnection.instances), 1)

    def test_real_urllib_response_timeout_closes_and_is_unknown_sent(self) -> None:
        response = FakeUrllibResponse(200, b"", read_error=TimeoutError())
        FakeHTTPSConnection.instances = []
        FakeHTTPSConnection.response = response
        FakeHTTPSConnection.request_error = None
        permit = FakePermit()
    ...
    def test_real_urllib_error_body_is_bounded_and_closed(self) -> None:
        response = FakeUrllibResponse(429, b"x" * (MAX_RESPONSE_BYTES + 1))
        FakeHTTPSConnection.instances = []
        FakeHTTPSConnection.response = response
        FakeHTTPSConnection.request_error = None
        permit = FakePermit()
    ...
    def test_real_urllib_redirect_is_terminal_without_second_invocation(self) -> None:
        ...
        self.assertFalse(result.ok)
        self.assertEqual(result.failure.category, "redirect_refused")
        self.assertEqual(result.failure.sent_state, SentState.SENT.value)
        self.assertEqual(permit.settle_calls[0][1], AttemptOutcome.PROVIDER_FAILED)
        self.assertEqual(len(FakeHTTPSConnection.instances), 1)
